...
**Key Insight**: Navigation visibility is UX only, not security. We can check everything client-side!
...
✅ **requireOrg**: Client-side check  
✅ **permission**: Client-side check  
✅ **role**: Client-side check  
...
Plans and features require server-side `has()` function. Options:
